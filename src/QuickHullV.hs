...
maxv :: VV2 -> V2
maxv points = VU.maximumBy (comparing fst) points

{-Function to find the point with the maximum x-coordinate-}
minv :: VV2 -> V2
minv points = VU.minimumBy (comparing fst) points

{-Furthest point from a line segment defined by points a1, a2-}
maxAreaPoint :: V2 -> V2 -> VV2 -> V2
maxAreaPoint anchor1 anchor2 points = VU.maximumBy (comparing (triArea anchor1 anchor2)) points

{-Groups by 2D determiants (cross product). Always return the points left of segment (a1,a2)-}
grouper :: V2 -> V2 -> VV2 -> VV2
grouper anchor1 anchor2 points = VU.filter (\z -> determinant anchor1 anchor2 z > 0) points
